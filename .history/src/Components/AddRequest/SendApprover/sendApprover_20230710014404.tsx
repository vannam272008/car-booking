...
function SendApprover(): JSX.Element {
    ...
    const [labelApprover, setLabelApprover] = useState("");

    const handleInputChangeApprover = (e) => {
        setLabelApprover(e.target.value);
    };

    const handleSave = () => {
        // Xử lý lưu thay đổi
        // ...
        setEditingIndex(-1); // Chuyển về chế độ xem thông tin
    };

    const handleEdit = (index) => {
        setLabelApprover(labelApprover); // Đặt giá trị của input ban đầu là giá trị hiện tại
        setEditingIndex(index); // Chuyển sang chế độ chỉnh sửa
    };
                                    ...
                                    <Form.Item
                                        label={
                                            <div>
                                                {editingIndex === index ? (
                                                    <Space>
                                                        <Input value={labelApprover} onChange={handleInputChange} />
