...
function SendApprover(): JSX.Element {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>, index: number, value: string) => {
    ...
    const [labelValue, setLabelValue] = useState('bangnm@o365.vn, Developer');
                                ...
                                <Col span={8} key={index}>
                                    <Form.Item
                                        label={
                                            <div>
                                                {isEditing ? (
                                                    <Space><Input value={labelValue} onChange={handleInputChange} /></Space>
                                                ) : (
                                                ...
                                                {isEditing ? (
                                                    ...
                                                    <Space>
                                                        ...
                                                        <Button type="link" icon={<SaveOutlined />} onClick={() => handleEdit(index)} />
                                                        <Button type="link" icon={<DragOutlined />} />
                                                    </Space>
                                                )}
                                                {/* Approver {index + 1} */}
                                    ...
                                    >
                                        <Select
                                            ...
                                            onChange={(value) => handleInputChange(e, index, value)}
                                            showSearch
                                            optionFilterProp="children"
                                            filterOption={(inputValue, option) =>
                                                option?.props.children?.toLowerCase().indexOf(inputValue.toLowerCase()) !== -1
