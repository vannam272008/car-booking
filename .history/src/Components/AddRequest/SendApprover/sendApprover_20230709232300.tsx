...
function SendApprover(): JSX.Element {
    ...
    const [labelValue, setLabelValue] = useState('bangnm@o365.vn, Developer'); // Giá trị ban đầu của nhãn

    const handleEdit = () => {
        setIsEditing(true); // Kích hoạt chế độ chỉnh sửa khi nút "Edit" được nhấn
    };
    ...
    const handleInputChange = (e) => {
        setLabelValue(e.target.value); // Cập nhật giá trị của nhãn khi người dùng thay đổi nó trong trường nhập liệu
    };
                    ...
                    <Form>
                        <Row gutter={16}>
                            <Col span={8}>
                                <Form.Item
                                    {isEditing ? (
                                        <Input value={labelValue} onChange={handleInputChange} /> // Hiển thị trường nhập liệu khi ở chế độ chỉnh sửa
                                    ) : (
                                        <span>{labelValue}</span> // Hiển thị nhãn khi không ở chế độ chỉnh sửa
                                    )}
                                    {isEditing ? (
                                        <Button onClick={handleSave}>Save</Button> // Hiển thị nút "Save" khi ở chế độ chỉnh sửa
                                    ) : (
                                        <Button onClick={handleEdit}>Edit</Button> // Hiển thị nút "Edit" khi không ở chế độ chỉnh sửa
                                    )}
                                    label={
                                        <div>
                                            Pho phong IT
