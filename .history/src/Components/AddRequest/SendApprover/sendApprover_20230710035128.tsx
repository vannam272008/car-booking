...
function SendApprover(): JSX.Element {
    ...
    const handleDragEnd = (dragIndex, hoverIndex) => {
        const newApprovers = [...approvers];
        const draggedApprover = newApprovers[dragIndex];

        newApprovers.splice(dragIndex, 1);
        newApprovers.splice(hoverIndex, 0, draggedApprover);

        setApprovers(newApprovers);
    };

    const DraggableCol = ({ index, children }) => {
        const ref = React.useRef(null);

        const [{ isDragging }, drag] = useDrag({
            item: { index, type: "APPROVER" },
            collect: (monitor) => ({
                            ...
                            <Row gutter={16}>
                                ...
                                <DndProvider backend={HTML5Backend}>
                                    {approvers.map((approver, index) => (
                                        <DraggableCol key={index} index={index}>
                                            <Form.Item
                                                label={
                                                        ...
                                                        {editingIndex === index ? (
                                                            <Space>
                                                                <Input
                                                                    ...
                                                                    onChange={(e) =>
                                                                        handleInputChangeApprover(index, e.target.value)
                                                                    }
                                                                />
                                                                <Button
                                                                    type="link"
