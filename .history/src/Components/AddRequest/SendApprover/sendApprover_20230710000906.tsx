...
function SendApprover(): JSX.Element {
    ...
    const [isEditingArray, setIsEditingArray] = useState(Array(inputs.length).fill(false));

    const handleEdit = (index: number) => {
        const updatedEditingArray = isEditingArray.map((item, i) => i === index);
        setIsEditingArray(updatedEditingArray);
    };
                                        ...
                                        label={
                                            <div>
                                                {isEditing && selectedItemIndex === index ? (
                                                    <Space>
                                                        <Input value={labelValue} onChange={handleInputChangeApprover} />
                                                        <Button icon={<SaveOutlined />} onClick={() => handleSave} />
                                                    </Space>
                                                ) : (
                                                    <div>
                                                        <span>{labelValue} {index + 1}</span>
                                                        <Space>
